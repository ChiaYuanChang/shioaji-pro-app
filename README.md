...
HTTP API + SSE streaming. React 19 + TypeScript + Vite for the trading
terminal path — it talks directly to your local `shioaji server`. Optional
strategy backtesting can connect to a local backtest service from a sibling
workbench repository.

以 Shioaji HTTP API 打造的專業交易終端：即時行情、K 線、五檔、閃電下單、
圖表點價下單、停損停利觸價單、可拖拉的自訂版面。

**介面 100% 開源** — UI、行情串流、下單鏈路都在這個 repo，clone 下來就能 build 出
完整的 Web 版終端。策略回測面板可連到外部本機回測服務；Python backend 不屬於這個 repo。
桌面版外殼（Tauri）與 AI Agent 為專屬模組，
直接到 Releases 下載安裝檔即可使用。
...
- **AI Agent** — 多供應商（Claude / Codex）agentic 對話、shioaji 技能市集、
  排程任務、操作觀察學習（桌面版專屬）
- **策略回測（已開源重建）** — 用 Python 寫進出場策略（與自訂指標同一套
  `ta` 函式集）、含手續費/證交稅/期交稅/滑價的回測引擎（訊號收盤成立、
  次根開盤成交，無未來函數）、單商品與多商品整合績效（合併權益曲線＋
  商品表）、進出場標記直接畫在 K 線上。開源版工作流程：啟動 sibling workbench 的
  回測服務（在目前本機 layout 可用 `cd ../backend && uv run python -m backtest`，預設
  `http://127.0.0.1:8787`），在終端加入「策略回測」面板即可使用；
  桌面版另附專屬回測面板。
...
> 桌面版外殼（Tauri）與 AI Agent 為專屬模組，不在本 repo ——
> 本 repo 可 build 出完整的 Web 版終端（CI 持續驗證），桌面版請直接下載安裝檔。
> 策略回測前端面板在本 repo；Python 回測服務由 sibling workbench repo 管理。
...
開啟 [http://localhost:5173](http://localhost:5173)。Vite dev server 預設把
`/api` 代理到開發 sidecar `127.0.0.1:21322`；若你自己跑 `shioaji server` 在 8080，
請用 `VITE_API_TARGET=http://127.0.0.1:8080 pnpm dev`。
